import { GameObject } from './GameObjects/types';
...
export default class GameRunner {
  ...
  startGame() {
    ...
    const ai = new DumbAI(this);
    const aiPaddle = new Paddle(this.canvas, 20, (this.canvas.height - 75)/2, ai);
    ai.setPaddle(aiPaddle);
    this.objects.push(aiPaddle);

    this.objects.push(new Paddle(this.canvas, this.canvas.width - 20, (this.canvas.height - 75)/2, new HumanController()));
  ...
  restartRound() {
    // @ts-ignore
    const ballIndex = this.objects.findIndex((o: GameObject) => o.name() === 'ball');
  ...
  ballHitSide(side: 'left' | 'right') {
    ...
    this.restartRound();
    if (this.score.getLeft() >= 7 || this.score.getRight() >= 7) {
      this.gameOver = true;
      this.renderGameOver();
      // end game
    }
